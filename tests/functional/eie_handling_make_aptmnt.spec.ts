...
// assert the count
 let drpdwnOptionsEle = page.getByLabel("Facility").locator('option')
  ...
  // Get dropdown values
let listOfDrpDownsElems = await page.getByLabel("Facility").all()
console.log('Dropdown menu  items:',listOfDrpDownsElems.length);
let listOfOptions = []
for (let ele of listOfDrpDownsElems){
  let eleTxt = ele.textContent()
  if (eleTxt){
    listOfOptions.push(eleTxt)
    console.log('Menuelement :', eleTxt);
  }
}const rowCount = await listOfOptions.length;
console.log('Number of elements',rowCount);
